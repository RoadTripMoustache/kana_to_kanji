...
GoRouter buildRouter([GlobalKey<NavigatorState>? key]) => GoRouter(
  ...
  routes: [
    ...
    GoRoute(path: SignInView.routeName, builder: (_, __) => const SignInView()),
    GoRoute(
    ...
    GoRoute(
      ...
      routes: [
        GoRoute(
          path: QuizView.routeName.substring(
            PrepareQuizView.routeName.length + 1,
          ),
          redirect: (_, state) {
          ...
          builder: (_, state) => QuizView(groups: state.extra! as List<Group>),
        ),
        GoRoute(
          path: QuizConclusionView.routeName.substring(
            PrepareQuizView.routeName.length + 1,
          ),
          redirect: (_, state) {
          ...
          builder:
              (_, state) =>
                  QuizConclusionView(questions: state.extra! as List<Question>),
        ),
      ],
    ),
    GoRoute(
