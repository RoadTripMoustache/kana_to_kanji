  ...
  group("FeedbackViewModel", () {
    ...
    group("Feedback type", () {
      test("selectedFeedbackType set by constructor", () {
        expect(
          viewModel.selectedFeedbackType,
          null,
    ...
    group("Form", () {
      ...
      test("onFormChange should update the corresponding value and "
          "call listeners", () {
        for (final field in FeedbackFormFields.values) {
          expect(
            viewModel.formData[field],
            "",
          ...
          expect(
            ...
            field.name,
            reason:
                "Should have been updated with the passed value, "
                "here the field name",
          );
        }
      ...
      group("formValidator", () {
        test("email", () {
          ...
          for (final entry in emails.entries) {
            expect(
              viewModel.formValidator(FeedbackFormFields.email, entry.key),
              entry.value,
              reason:
                  "${entry.key} should be "
        ...
        test("description", () {
          ...
          expect(
            ...
            reason:
                "Null description isn't allowed when "
                "feedback is feature request",
          );
          expect(
            viewModel.formValidator(FeedbackFormFields.description, ""),
            l10n.feedback_empty_description,
            reason:
                "Empty description isn't allowed when "
                "feedback is feature request",
          );
          expect(
        ...
        test("steps to reproduce", () {
          ...
          viewModel.onFeedbackTypePressed(FeedbackType.bug);
          expect(
            viewModel.formValidator(FeedbackFormFields.stepsToReproduce, null),
      ...
      test("onIncludeScreenshotPressed", () {
        ...
        when(
          contextMock.dependOnInheritedWidgetOfExactType<FeedbackData>(),
      ...
      group("onFormSubmit", () {
        setUp(() {
          when(
            infoServiceMock.devicePlatformName,
        ...
        test(
          "should create the github issue, close the popup and show success",
          () async {
            // Prepare the model
            viewModel.onFeedbackTypePressed(FeedbackType.featureRequest);
            viewModel.formData.update(
            ...
            expect(
              capturedArgs[0],
              buildIssueTitle(FeedbackType.featureRequest),
            );
            expect(
              capturedArgs[1],
              buildIssueBody(appConfig.environment, viewModel.formData),
            );
            expect(capturedArgs[2], <String>[
              FeedbackType.featureRequest.value,
            ]);
        ...
        test(
          "should upload image, create the github issue, and show success",
          () async {
            // Get an image
            ...
            // Stub returns
            when(
              githubServiceMock.uploadFileToGithub(
